...
export default function IndividualPlaylistPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchPlaylistData = async () => {
    try {
      ...
      const response = await fetch(`/api/playlist/get-individual-playlist?id=${playlistId}`)
        ...
        playlistData.items.map(async (item: PlaylistItem) => {
          try {
            ...
            if (playlistData.type === 'SONG') {
              ...
              const songResponse = await fetch(`/api/get-song-data?id=${item.itemId}`)
              if (songResponse.ok) {
              ...
              const movieResponse = await fetch(`/api/get-movie-data?id=${item.itemId}`)
              if (movieResponse.ok) {
      ...
      console.log('Fetched playlist with real names:', { ...playlistData, items: itemsWithNames })
    ...
    return (
      <div className={`playlist-page-container ${getPlaylistPageTheme()}`}>
        ...
        <div className="error-container">
          ...
          <button onClick={() => router.push('/playlist')} className="back-button">
            Back to Playlists
          </button>
        </div>
      </div>
          ...
          <div className="playlist-header">
            ...
            <div className="playlist-title-section">
              ...
              <p className="playlist-info">
                {playlist.items.length} {playlist.type.toLowerCase()}{playlist.items.length !== 1 ? 's' : ''}
              </p>
            </div>
          </div>
