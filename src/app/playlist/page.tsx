      ...
      <main className="playlist-main">
        <div className="playlist-content">
          ...
          <div className="playlist-items-section">
            {(filterType === 'SONG' ? musicPlaylists.length : 
              filterType === 'MOVIE' ? moviePlaylists.length : 
              totalPlaylists) === 0 ? (
              <div className="empty-playlist">
                <p>You haven't created any {filterType === 'SONG' ? 'music' : filterType === 'MOVIE' ? 'movie' : ''} playlists yet</p>
                <p className="empty-subtitle">
                  Start adding {filterType === 'SONG' ? 'songs' : filterType === 'MOVIE' ? 'movies' : 'songs and movies'} to create your first playlist!
                </p>
              </div>
            ) : (
              <>
                {/* Music Playlists Section - only show if not filtered to MOVIE only */}
                {(!filterType || filterType === 'SONG') && musicPlaylists.length > 0 && (
                  <div className="playlist-section">
                    <h2 className="section-title">🎵 Music Playlists ({musicPlaylists.length})</h2>
                    <div className="playlist-items-grid">
                      {musicPlaylists.map((playlist) => (
                        ...
                        >
                          <div className="item-icon">{getPlaylistTypeIcon(playlist.type)}</div>
                ...
                {(!filterType || filterType === 'MOVIE') && moviePlaylists.length > 0 && (
                  <div className="playlist-section">
                    <h2 className="section-title">🎬 Movie Playlists ({moviePlaylists.length})</h2>
                    <div className="playlist-items-grid">
