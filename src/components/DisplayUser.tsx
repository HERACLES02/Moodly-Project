...
export default function DisplayUser({
  ...
  const fetchOtherUserData = async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.error(
          "❌ DisplayUser: Failed to fetch user data, status:",
          response.status,
        )
        setUserData(null)
      }
  ...
  const getAvatarDisplay = () => {
    if (userData.currentAvatar && userData.currentAvatar.imagePath) {
      return (
        <Avatar className="rounded-2xl size-10 ">
          <AvatarImage src={imgPath} />
          <AvatarFallback>{userData.currentAvatar.name}</AvatarFallback>
        </Avatar>
      )
    } else {
  ...
  return (
    <div
      className={`display-user hover:bg-transparent`}
      onMouseEnter={handleMouseEnter}
      onMouseLeave={handleMouseLeave}
    >
      {/* Avatar Section */}
      {showAvatar && <div className="">{getAvatarDisplay()}</div>}

      {/* Name Section */}

      {/* Tooltip for notes (when note prop is true) */}
      {note && showTooltip && userData.note && (
        <div className="user-note-tooltip">{userData.note}</div>
      )}
    </div>
  )
}
