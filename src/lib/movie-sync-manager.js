class MovieSyncManager {
  ...
  async initializeQueue() {
    try {
      const response = await fetch(
        `http://localhost:9513/api/recommendations/movies?mood=${this.mood}`,
      )
      ...
      if (data.movies && data.movies.length > 0) {
        this.movieQueue = data.movies.map((movie) => ({
          ...
          title: movie.title,
          vidsrcUrl: `https://www.vidking.net/embed/movie/${id}?progress=120`,
        ...
        console.log(
          `✅ ${this.mood} mood queue initialized with ${this.movieQueue.length} movies`,
        )
        return true
      } else {
      ...
      console.error(
        `Failed to initialize ${this.mood} queue, using fallback:`,
        error,
      )
      this.movieQueue = this.getFallbackMovies(this.mood)
      return true
  ...
  getFallbackMovies(mood) {
    const happyMovies = [
      {
        ...
        overview:
          "A cowboy doll is profoundly threatened when a new spaceman toy supplants him as top toy in a boy's room.",
      },
      {
        id: "585",
        title: "Monsters, Inc.",
        ...
        duration: this.movieDuration,
        poster:
          "https://image.tmdb.org/t/p/w500/wdWwtvkRRlgTiUr6TyLSMixTQT.jpg",
        overview:
          "Lovable Sulley and his wisecracking sidekick Mike Wazowski are the top scare team at Monsters, Inc.",
      },
      {
        id: "121",
      ...
      },
      {
        ...
        overview:
          "Harry Potter learns on his 11th birthday that he is the orphaned son of two powerful wizards.",
      },
    ]
    ...
    const sadMovies = [
      ...
      },
      {
        ...
        overview:
          "The presidencies of Kennedy and Johnson through the eyes of Alabama man with an IQ of 75.",
      },
      {
        id: "238",
        title: "The Godfather",
  ...
  scheduleNextMovie() {
    ...
    console.log(
      `⏰ ${this.mood} mood: Next movie scheduled in ${this.movieDuration / 60000} minutes`,
    )
  }
  ...
  getCurrentSessionInfo() {
    ...
    return {
      ...
      nextMovie:
        this.movieQueue[(this.currentIndex + 1) % this.movieQueue.length],
      mood: this.mood,
      syncData: {
  ...
  addViewer(socketId, username, mood) {
    ...
    console.log(
      `👤 ${username} joined ${this.mood} stream (${this.viewers.size} total viewers)`,
    )
    return this.getCurrentSessionInfo()
  }
  ...
  removeViewer(socketId) {
    ...
    if (viewer) {
      ...
      console.log(
        `👋 ${viewer.username} left ${this.mood} stream (${this.viewers.size} total viewers)`,
      )
    }
  }
