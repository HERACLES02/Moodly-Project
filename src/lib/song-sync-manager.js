class SongSyncManager {
  ...
  async initializeQueue() {
    try {
      ...
      const response = await fetch(
        `http://localhost:9513/api/recommendations/songs?mood=${this.mood}`
      )
      ...
      if (data.tracks && data.tracks.length > 0) {
        ...
        console.log(
          `${this.mood} mood song queue initialized with ${this.songQueue.length} songs`
        )
        console.log(
          `🎵 First song: ${this.songQueue[0].title} by ${this.songQueue[0].artist}`
        )
        return true
      } else {
        console.log(
          `${this.mood} song API returned no tracks, using fallback songs`
        )
    ...
    const sadSongs = [
      {
        ...
        previewUrl:
          "https://p.scdn.co/mp3-preview/someone-like-you-preview.mp3",
        duration: this.songDuration,
  ...
  scheduleNextSong() {
    ...
    console.log(
      `🎵 ${this.mood} radio: Next song scheduled in ${
  ...
  switchToNextSong() {
    ...
    console.log(
      `${this.mood} radio switching to: ${this.currentSong.title} by ${this.currentSong.artist}`
    )
  ...
  addListener(socketId, username, mood) {
    ...
    console.log(
      `🎧 ${username} joined ${this.mood} radio (${this.listeners.size} total listeners)`
    )
    return this.getCurrentSessionInfo()
  }
  ...
  removeListener(socketId) {
    ...
    if (listener) {
      ...
      console.log(
        `${listener.username} left ${this.mood} radio (${this.listeners.size} total listeners)`
      )
    }
  }
}
